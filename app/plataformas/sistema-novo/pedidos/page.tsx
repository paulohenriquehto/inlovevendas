...
export default function SistemaNovoPedidosPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const data = await loadCSVData('sistema-novo');
  ...
  const handleOrderClick = async (orderNumber: string) => {
    try {
      const allData = await loadCSVData('sistema-novo');
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="space-y-1">
          <h1 className="text-3xl font-bold tracking-tight">Pedidos - Sistema Novo</h1>
