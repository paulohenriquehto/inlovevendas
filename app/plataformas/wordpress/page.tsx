...
export default function WordPressDashboard() {
  ...
  const { data: allVendas, isLoading: loading, isError } = useWordPressVendas();

  // Calcular vendas filtradas e métricas usando useMemo para evitar recalculos desnecessários
  const { vendas, metrics } = useMemo(() => {
    ...
    const calculatedMetrics = calculateWordPressMetrics(filteredVendas);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-xl">Carregando dados do WordPress...</div>
      </div>
    );
  }
  ...
  if (!metrics) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-xl text-red-600">Erro ao carregar dados do WordPress</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="space-y-2">
          ...
          <h1 className="text-4xl font-bold tracking-tight">Dashboard WordPress</h1>
          <p className="text-muted-foreground">
            Análise completa das vendas com {metrics.totalPedidos.toLocaleString('pt-BR')} pedidos únicos
            {selectedPeriod !== 'all' && (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <MetricCard
            ...
            value={Math.round(metrics.totalPedidos).toLocaleString('pt-BR')}
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={metrics.topClientes} layout="vertical">
                ...
                <Tooltip
                  formatter={(value: number, name: string) => {
                    if (name === 'totalGasto') return formatCurrency(value);
                    return value;
                  }}
                />
                <Legend />
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Card>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
                    data={metrics.distribuicaoClientes}
                    cx="50%"
                    cy="50%"
                    labelLine={false}
                    label={({ name, percent }) =>
          ...
          {/* Análise de Cupons (se houver) */}
          {metrics.usoCupons && metrics.usoCupons.length > 0 && (
            <Card>
              <CardHeader>
